...
```
- crontab -l 2>/dev/null: Liệt kê các tác vụ tự động hiện có của người dùng. Nếu chưa có tác vụ nào, nó sẽ đẩy lỗi vào "hố đen" /dev/null để tránh làm hỏng lệnh tiếp theo.

- echo "@reboot /sbin/swapoff -a": Tạo ra một dòng cấu hình mới: "Mỗi khi máy khởi động lại (@reboot), hãy chạy lệnh tắt swap (/sbin/swapoff -a)".

- | crontab -: Lấy toàn bộ danh sách cũ cộng với dòng mới vừa tạo để ghi đè ngược lại vào bảng quản lý tác vụ (crontab).

- || true: Đảm bảo rằng dù lệnh có gặp lỗi nhỏ (ví dụ: crontab trống), script vẫn sẽ chạy tiếp mà không dừng lại.

3. Load modules cho CRI-O
```
